...
sed -i "s/<VERSION>/$(echo $CIRCLE_TAG | cut -c 1-16)/" template/deployment.yml
sed -i "s/<REPO>/$(echo $REPO | cut -c 1-16)/" template/deployment.yml
sed -i "s/<PROJECT>/$(echo $CIRCLE_PROJECT_REPONAME | cut -c 1-16)/" template/deployment.yml
